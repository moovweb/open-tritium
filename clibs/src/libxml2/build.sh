...
[ ! -z $DEBUG_MODE ] && DEBUG_FLAGS="--with-debug --with-run-debug"

# make an output folder just for this lib
rm -rf "$CLIBS_HOME/output/libxml2"
mkdir -p "$CLIBS_HOME/output/libxml2"

# Build this lib after building iconv to correctly bind it.
#
./autogen.sh --prefix="$CLIBS_HOME/output/libxml2" --with-iconv=$CLIBS_HOME $DEBUG_FLAGS
make install
